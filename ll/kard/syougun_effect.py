...
class SyougunEffect():
    def __init__(
            self, bridge: Bridge, guards_async: Callable[[], None] | None=None,
            exchange_kards_async: Callable[[Player, Player], None] | None=None
    ) -> None:
        ...
        '''
        self.bridge = bridge
        self.guards_async: Callable[[], None] = guards_async if guards_async else\
            lambda : guard_message(bridge=bridge, kard_name="将軍")
        self.exchange_kards_async: Callable[[Player, Player], None] = exchange_kards_async\
            if exchange_kards_async else ExchangeKardsController(bridge=bridge).action
    ...
    def commit_board(self, board: Board, p1: Player, p2: Player) -> None:
        '''Boardのデータを実際に更新する命令。
