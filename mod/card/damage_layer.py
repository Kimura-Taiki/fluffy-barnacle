...
def _open(layer: PipelineLayer, stat: PopStat) -> None:
    if not isinstance(da := layer.card, _DamageArrow):
...
#                 20                  40                  60                 79
def damage_layer(card: Card, delivery: Delivery, hoyuusya: int, code: int) -> PipelineLayer:
    if not isinstance(da := card, _DamageArrow):
        raise EOFError
...
# start_phase_layer: Callable[[Delivery, int, _DamageArrow, int], PipelineLayer] =\
