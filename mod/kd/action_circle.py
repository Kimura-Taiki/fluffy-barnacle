...
def mouseup(youso: Youso, mode: int=OBAL_KIHONDOUSA) -> None:
    ...
    if int((diff_coord.angle_to([0, 0])+225)/90) == 3:
        ...
        moderator.append(standard_basic_action_layer(cards=[yadosi_card],
            huda=enforce(youso, Huda)))
    elif int((diff_coord.angle_to([0, 0])+225)/90) == 1:
        moderator.append(standard_basic_action_layer(cards=_bacs,
            huda=enforce(youso, Huda)))
#                 20                  40                  60                 79
    else:
        moderator.append(standard_basic_action_layer(cards=[zensin_card],
            huda=enforce(youso, Huda)))
