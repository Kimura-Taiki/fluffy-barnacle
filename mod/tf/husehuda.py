...
def _husehuda_draw(huda: Huda) -> None:
    ...
    img_rz_topleft = huda.huda_draw.img_rz_topleft
    for i in range(19):
        IMG_BACK.set_alpha(i*25+30)
        screen.blit(source=IMG_BACK, dest=img_rz_topleft+[0, i*20], area=(0, i*20, 285, 20))
    IMG_BACK.set_alpha(255)
    screen.blit(source=IMG_BACK, dest=img_rz_topleft+[0, 180], area=(0, 180, 285, 24))
    for i in range(4):
        img_rz.set_alpha(i*48+48)
        screen.blit(source=img_rz, dest=img_rz_topleft+[0, i*9+132], area=(0, i*9+132, 285, 9))
    img_rz.set_alpha(224)
    screen.blit(source=img_rz, dest=img_rz_topleft+[0, 168], area=(0, 168, 285, 36))
